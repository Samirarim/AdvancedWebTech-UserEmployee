...
import { CookInfo, CookUpdateInfo } from '../cook/cook.dto';
import { CookService } from '../cook/cook.service';
import { DMInfo, DMUpdateInfo } from '../deliveryman/deliveryman.dto';
import { DMService } from '../deliveryman/deliveryman.service';
import { WaiterInfo, WaiterUpdateInfo } from '../waiter/waiter.dto';
...
signup(@Body() manager:ManagerInfo,@UploadedFile(  new ParseFilePipe({
  validators: [
    new MaxFileSizeValidator({ maxSize: 320000000 }),
    new FileTypeValidator({ fileType: 'png|jpg|jpeg|' }),
  ],
}),) file: Express.Multer.File){
return this.managerService.signup(manager, file.filename);
}*/

@Post('/signin')
//@UsePipes(new ValidationPipe())
async signin(@Session() session, @Body() manager:ManagerInfo)
  {
    const res = await (this.managerService.signin(manager));
    if(res == true)
    {
      session.m_email = manager.m_email;
      return (session.m_email);
    }
    else
    {
  ...
  //@UseGuards(SessionGuard)
  @UsePipes(new ValidationPipe())
  changePass(@Session() session,@Body('m_password') m_password: string): any {
    console.log(session.email);
    return this.managerService.changePass(m_password, session.m_email);
  }
...
@Get('/getwaiter/:w_id')
  getWaiterByID(@Param('w_id', ParseIntPipe) w_id: number): any {
    return this.waiterService.getWaiterByID(w_id);
  }
  ...
  //@UseGuards(SessionGuard)
  @UsePipes(ValidationPipe)
  updateWaiter(@Param('w_id',ParseIntPipe) w_id: any , @Body() waiter: WaiterUpdateInfo): any {
 ...
 //@UseGuards(SessionGuard)
  ...
  @Get('/getcook/:c_id')
  getCookByID(@Param('c_id', ParseIntPipe) c_id: number): any {
    return this.cookService.getCookByID(c_id);
  }
  ...
  @Get('/getdeliveryman/:dm_id')
  getDMByID(@Param('dm_id', ParseIntPipe) dm_id: number): any {
    return this.dmService.getDMByID(dm_id);
  }
